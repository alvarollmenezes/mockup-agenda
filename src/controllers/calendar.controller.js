...
module.exports = () => {
  ...
  calendarController.availableCalendars = (req, res) => {
    return Promise.resolve(calendarService.getAvailableCalendars()).then(data =>
      res.json(data)
    );
  };
  ...
  calendarController.events = (req, res) => {
    ...
    return calendarService.getEvents(params).then(events => res.json(events));
  };

  calendarController.events2 = (req, res) => {
    let params = req.query;

    return calendarService.getEvents2(params).then(events => res.json(events));
  };
  ...
  calendarController.eventsGovEs = (req, res) => {
    ...
    return calendarService
      .getGovEsEvents(query, maxResults)
