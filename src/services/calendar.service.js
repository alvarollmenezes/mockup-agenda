...
module.exports = () => {
  listEvents = (params, normalizer) => {
    ...
    return Promise.all(promises).then(events =>
      _.flatten(events.map(normalizer))
    );
  };
  ...
  normalizeCalendarGovES = calendar => calendar.data.items;

  normalizeCalendar = (calendar, index) => ({
    color: calendarConfig.colors[index],
    summary: calendar.data.summary,
    etag: calendar.data.etag,
    items: calendar.data.items.map(event => ({
      color: calendarConfig.colors[index],
      start: event.start,
      end: event.end,
  ...
  calendarService.getAvailableCalendars = () => {
    return Object.keys(dbCalendars)
      .map((k, i) => {
        return { name: k, color: calendarConfig.colors[i] };
      })
      .sort();
  };
  ...
  calendarService.getGovEsEvents = (query, maxResults) => {
    return listEvents(query, normalizeCalendarGovES).then(events => {
      console.log(events);
      return events
